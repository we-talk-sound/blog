      ...
      preClick={() => {
        story?.item &&
          dispatch({
            ...
            payload: { ref: 'blogSingleStories', items: [story?.item] }
          });
      }}
    >
      <>
        {storyImageCondition && !bannerImageCondition && <img src={story.image} alt={`story-image`} />}

        <span className="story-item-focuser" />

        <div className="story-item-body">
          <div className="story-item-heading">
            <span className="story-item-category">{story?.category}</span>
          ...
          {subCondition && story.sub && (
            <div className="story-item-sub" dangerouslySetInnerHTML={{ __html: story.sub }} />
          )}
